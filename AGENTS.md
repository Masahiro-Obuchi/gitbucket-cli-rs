...
- Implemented top-level commands: `auth`, `repo`, `issue`, `pr`, `browse`, `config`, `api`, `label`, and `milestone`.
...
- Planned but not yet implemented: `completion`.

## Project Structure & Module Organization

- `src/main.rs`: CLI entrypoint and command dispatch.
- `src/cli/`: command implementations (`auth`, `api`, `config`, `label`, `milestone`, `repo`, `issue`, `pr`) and shared resolution logic in `common.rs`.
